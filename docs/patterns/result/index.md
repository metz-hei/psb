...
## Состояние «Ошибка»
Экраны ошибок показывают, что что-то пошло не так. Где-то моргунул сервер, отвалилась интеграция или попались «неправильные» данные. Наша задача помочь пользователю выбраться из этой ситуации и вернуться к своему сценарию.
...
Структура страницы и обязательность элементов наследуется из успешного сценария.

![Платёжная страница. Блокирующая ошибка](./14.png)

Если сценарий завершился не блокирующей ошибкой и пользователь может повлиять на её устранение, например, вернуться в форму и исправить данные, то структура страницы меняется.
