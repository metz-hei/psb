...
* Нет перелинковки с бэклогом по компоненту: непонятно когда будет реализован компонент, взят ли он в работу, какие дефекты и доработки есть по реализованному компоненту, какие приоритеты на реализацию. На самом деле, это проблема не столько документации, сколько именно бэклога. Документация лишь хранит связь со всем этим, но не отвечает за это. К бэклогу мы вернемся позже.
...
### Решение
Внедрить в релизный цикл работу со опенсорсным решением — Storybook. Он уже используется для вебовских компонентов, но доступен только локально. Надо:
* Интегрировать в Storybook мобильную витрину, чтобы уменьшить количество используемых инструментов, унифицировать и стандартизировать процесс.
* Расшарить Storybook на всех участников производственного цикла и связать со всеми остальными инструментами путём добавления перелинковки.
* Договориться о содержимом и передать управление пропсами в руки дизайнеров.
...
4. Встроенная функция комментирования компонентов, аналогичная Фигме. Проходить дизайн-ревью, можно так же быстро и удобно как инспекцию в Фигме.
5. Возможность внедрения регрессионного тестирования компонентов с помощью автотестов через Chromatic. Это логичный следующий шаг развития Сторибука, который позволит ускорить процесс тестирования (выявлять проблемы на ранних этапах, быстрее проводить регресс) и повысит уверенность в сборках. Так как тесты визуализируются в хроматике, принимать решения о принятии изменений или их отклонении могут помогать дизайнеры.
...
### Решение 
* Объединить бэклоги дизайнеров и фронтов. 
* Работать над компонентом в рамках одной задачи.
...
4. Переезжая в общий бэклог, следует отказаться от Notion по причине зависимости от платформы. Наш бэклог, это большая ценность, мы не можем рисковать его потерей.
...
3. GitLab
    * Бэклог задач на доработку дизайн-системы, баги и технический долг.
    * Исходный код Docusaurus, позволяет проводить ревью документации и деплоит её. 
    * Исходный код дизайн-системы.
    * Релиз-ноутсы, например как у [Вконтакте](https://github.com/VKCOM/VKUI/releases). Такой подход обеспечивает связь со всеми артефактами релиза: версией, задачей, документацией, пулл реквестом и избавляет нас от инструмента в виде телеграмма.  
...
<iframe width="848" height="517" src="https://www.youtube.com/embed/5amlQ57G10c" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
...
<iframe width="848" height="517" src="https://www.youtube.com/embed/Lk1_kkvVkLc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
