...
### Решение 
Необходимо объединить бэклоги дизайнеров и фронтов, чтобы работать над компонентом в рамках одной задачи и внести изменение в регламент движения задач.
...
### Бэклог
1. Описать новый регламент движения задач, определиться со статусной моделью.
...
4. Одна страница = один компонент. Состояния компонента меняются через пропсы.
5. Набор нужных пропсов дизайнер и разработчик определяют совместно. Это должно биться с Фигмой.
6. Структура Storybook должна матчиться со структурой Фигмы.
7. Chromatic должен быть включен в релизный цикл каждой задачи, если коспонент не покрыт тестами — задача не сделана.
